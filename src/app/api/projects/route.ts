...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: project, error: projErr } = await supabase
      .from("projects").insert({ title, genre, summary, status: "writing", user_id: user.id }).select().single();
    if (projErr) return Response.json({ error: projErr.message }, { status: 500 });

    if (outline) {
      const volMatches = [...outline.matchAll(/第([一二三四五])卷[：:\s]*([^\n]*)/g)];
      ...
      for (let i = 0; i < volMatches.length; i++) {
        ...
        const { data: volume } = await supabase
          .from("volumes").insert({ project_id: project.id, title: volNames[i], sort_order: i + 1 }).select().single();
        if (!volume) continue;
